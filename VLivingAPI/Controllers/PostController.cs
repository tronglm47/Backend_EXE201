...
namespace VLivingAPI.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        // POST api/Post/upload - Create post with file uploads
        [Produces("application/json", "application/xml")]
        [HttpPost("upload")]
        [Consumes("multipart/form-data")]
        [BusinessAuthorize(BusinessRole.PostManagement)] // Users who can create posts
        public async Task<IActionResult> PostWithFiles([FromForm] PostRequest.CreatePostWithFiles request)
        {
            try
            {
                ...
                var postId = await _postService.CreateWithFiles(request);
                if (postId <= 0)
                {
                    return BadRequest(new
                    {
        ...
        // PUT api/Post/5/upload - Update post with file uploads
        [Produces("application/json", "application/xml")]
        [HttpPut("{id}/upload")]
        [Consumes("multipart/form-data")]
        [BusinessAuthorize(BusinessRole.PostManagement)] // Users who can edit posts
        public async Task<IActionResult> PutWithFiles(int id, [FromForm] PostRequest.UpdatePostWithFiles request)
        {
            try
            {
                if (id <= 0)
                ...
                var result = await _postService.UpdateWithFiles(request, id);
                if (!result)
                {
                    return NotFound(new { success = false, message = "Post not found" });
                }
                ...
                {
                    success = true,
                    message = "Post updated successfully with uploaded images"
                });
            }
            catch (ArgumentException ex)
            {
        ...
        // PUT api/Post/5/images - Update only post images
        [Produces("application/json", "application/xml")]
        [HttpPut("{id}/images")]
        [Consumes("multipart/form-data")]
        [BusinessAuthorize(BusinessRole.PostManagement)] // Users who can edit posts
        public async Task<IActionResult> PutImages(int id, [FromForm] PostRequest.UpdatePostImages request)
        {
            try
            {
                if (id <= 0)
                ...
                var result = await _postService.UpdateImages(request, id);
                if (!result)
                {
                    return NotFound(new { success = false, message = "Post not found" });
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    success = false,
                    message = "Internal server error occurred while updating the post images",
                    error = ex.Message
                });
            }
        }
        ...
        #region Old Methods (to be removed later)
        /*
         // POST api/Post
        [Produces("application/json", "application/xml")]
        [HttpPost]
